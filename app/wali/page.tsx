...
export default function WaliDashboard() {
  ...
  // Fungsi Tanda Tangan Digital yang Diperbarui
  const handleSignPpi = async (ppiId: string) => {
    ...
    if (error) {
      ...
      // Update state modal secara instan agar tombol berubah jadi hijau
  ...
  if (loading) {
    return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-white">Memuat Dashboard Wali Siswa...</div>;
  }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white/5 border border-white/10 p-6 rounded-3xl backdrop-blur-xl gap-4 shadow-2xl">
          <div className="flex items-center gap-4">
            <div className="p-3.5 bg-gradient-to-br from-emerald-600/30 to-blue-600/30 border border-emerald-500/30 rounded-2xl shadow-inner">
              <Users className="w-8 h-8 text-emerald-400" />
            </div>
            <div>
              <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-blue-400">
                Dashboard Wali Siswa - SDIRF
              </h1>
              <p className="text-slate-400 text-sm">Orang Tua/Wali dari: <strong className="text-white">{childInfo}</strong></p>
            </div>
          </div>
          
          <button 
            onClick={handleLogout}
            className="flex items-center gap-2 bg-red-500/10 border border-red-500/30 hover:bg-red-500/20 text-red-400 px-4 py-2.5 rounded-xl transition-all text-sm font-semibold"
          >
        ...
        {/* WELCOME CARD */}
        <div className="p-8 rounded-3xl bg-gradient-to-r from-emerald-900/20 to-blue-900/20 border border-white/10 backdrop-blur-xl shadow-xl">
          <h2 className="text-3xl font-extrabold mb-2">Selamat Datang, {parentName || "Wali Siswa"}! 👋</h2>
        ...
        {activeTab === "laporan" && (
          <div className="flex flex-col gap-6">
            <h3 className="text-xl font-bold text-white flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-emerald-400" /> Riwayat Laporan Pembelajaran Harian & Tanggapan Orang Tua
            </h3>
        ...
        {activeTab === "ppi" && (
          <div className="flex flex-col gap-6">
            <h3 className="text-xl font-bold text-white flex items-center gap-2">
              <FileText className="w-5 h-5 text-blue-400" /> Program Pembelajaran Individual (PPI) & Persetujuan
            </h3>
      ...
      {selectedReport && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-white/20 rounded-3xl p-6 md:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative space-y-4">
            ...
            <h3 className="text-xl font-bold text-emerald-400 border-b border-white/10 pb-3">Detail Laporan Harian Pembelajaran</h3>
            ...
            <div className="space-y-4 text-sm text-slate-300">
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
                <div>
                  ...
                  <p className="text-white bg-white/5 p-3 rounded-lg">{selectedReport.materi_pembelajaran || "-"}</p>
                </div>
                <div>
                  <h4 className="text-xs uppercase text-slate-400 font-semibold mb-1">Target Capaian</h4>
                  <p className="text-white bg-white/5 p-3 rounded-lg">{selectedReport.target_capaian || "-"}</p>
                </div>
                <div>
                  <h4 className="text-xs uppercase text-emerald-400 font-semibold mb-1">Hasil Pembelajaran</h4>
                  <p className="text-emerald-200 bg-emerald-500/10 border border-emerald-500/20 p-3 rounded-lg">{selectedReport.hasil_pembelajaran || "-"}</p>
                </div>
                {selectedReport.catatan_perilaku && (
                  <div>
                    <h4 className="text-xs uppercase text-amber-400 font-semibold mb-1">Catatan Perilaku</h4>
                    <p className="text-amber-200 bg-amber-500/10 border border-amber-500/20 p-3 rounded-lg">{selectedReport.catatan_perilaku}</p>
                  </div>
                )}
                {selectedReport.intervensi_pendamping && (
                  <div>
                    ...
                    <p className="text-purple-200 bg-purple-500/10 border border-purple-500/20 p-3 rounded-lg">{selectedReport.intervensi_pendamping}</p>
                  </div>
                )}
              </div>
            </div>

            <button onClick={() => setSelectedReport(null)} className="w-full bg-slate-800 hover:bg-slate-700 text-white py-3 rounded-xl font-semibold">
              Tutup Detail
            </button>
          </div>
        </div>
      ...
      {selectedPpi && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-white/20 rounded-3xl p-6 md:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative space-y-4">
            ...
            <h3 className="text-xl font-bold text-blue-400 border-b border-white/10 pb-3">Detail Dokumen Program Pembelajaran Individual (PPI)</h3>
            ...
            <div className="pt-4 border-t border-white/15">
              {!selectedPpi.ttd_orangtua ? (
                <button 
                  ...
                  className="w-full bg-gradient-to-r from-emerald-600 to-blue-600 hover:opacity-90 text-white py-3 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2"
                >
                  <CheckCircle className="w-5 h-5" /> Berikan Tanda Tangan Digital & Setujui PPI
                </button>
              ) : (
                <div className="w-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 py-3 rounded-xl font-semibold text-center">
